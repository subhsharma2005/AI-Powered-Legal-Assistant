...
- Strong dependency on external services (Supabase/Postgres and Gemini API).
- No authentication/rate limiting on the Flask endpoint.
- Retrieval and prompting behavior are mostly hardcoded (e.g., top-80 retrieval in API).
- Database schema/table (`legal_chunks`) is assumed to exist with expected columns.
- Model loading happens at startup and can be slow/heavy on CPU-only systems.
- Some helper scripts are experimental/debug-oriented and not production-hardened.

---

## Notes for Contributors / Extension Ideas

- Add migration scripts (or SQL schema docs) for `legal_chunks` to simplify first-time setup.
- Add tests for ingestion, retrieval, and API error handling.
- Introduce configurable retrieval parameters (top-k, role filters, prompt templates).
- Add observability (structured logs, tracing) and API hardening (auth, throttling).
- Consider citation formatting tied to chunk IDs/case IDs for better auditability.

---

## Disclaimer

This project is for educational/research assistance and is **not** a substitute for professional legal advice.
